...
cd /data/SCCData/noaa-data #https://github.com/CareyLabVT/SCCData/tree/noaa-data
...
cd /data/SCCData/fcre #https://github.com/CareyLabVT/noaa_gefs_forecasts/tree/fcre
#git pull &>> ~/data/SCCData/noaa-logs/git.log
git add .
git commit -m "$TIMESTAMP: Git Backup" 2>&1 | tee -a $LOGFILE
/data/FLARE/git-retry.sh push -f 2>&1 | tee -a $LOGFILE

cd /data/noaa_gefs_forecasts/sugg #https://github.com/CareyLabVT/noaa_gefs_forecasts/tree/sugg
#git pull &>> ~/data/SCCData/noaa-logs/git.log
git add .
git commit -m "$TIMESTAMP: Git Backup" 2>&1 | tee -a $LOGFILE
/data/FLARE/git-retry.sh push -f 2>&1 | tee -a $LOGFILE

cd /data/noaa_gefs_forecasts/sunp #https://github.com/CareyLabVT/noaa_gefs_forecasts/tree/sunp
#git pull &>> ~/data/SCCData/noaa-logs/git.log
git add .
git commit -m "$TIMESTAMP: Git Backup" 2>&1 | tee -a $LOGFILE
/data/FLARE/git-retry.sh push -f 2>&1 | tee -a $LOGFILE
